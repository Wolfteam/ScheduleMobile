 ...
 */
class SemestresSpinnerAdapter(context: Context, resource: Int) :
    BaseSpinnerAdapter<SemestreDTO>(context, resource) {

    override fun getItemId(position: Int): Long {
        return mItemsList[position].idSemestre
    }

    override fun getItem(itemID: Long): SemestreDTO {
        return mItemsList.first { it.idSemestre == itemID }
    }

    override fun getPosition(itemID: Long): Int {
        return mItemsList
            .takeWhile { it.idSemestre != itemID }
            .count()
    }
    ...
    override fun createItemView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        holder.tvTextToShow?.text = mItemsList[position].nombreSemestre
        return rowView
    }
}