 ...
 */
class ProfesoresFragment : ItemBaseFragment<ProfesorDetailsDTO>(), ProfesoresViewContract,
    ...
    override fun initLayout(view: View?, savedInstanceState: Bundle?) {
        ...
        mAdapter = ProfesoresListAdapter(this)
        editardb_fragment_common_recycler_view.adapter = mAdapter as ProfesoresListAdapter
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == EDITARDB_DETAILS_REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                val item = data?.getParcelableExtra<ProfesorDetailsDTO>(Constants.ITEM_TAG)
                when (operation) {
